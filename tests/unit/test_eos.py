...
@given(p0   = st.floats(min_value = 30000, max_value = 101300), # pressure between 1013hPa - 300hPa
...
@given(z  = st.floats(min_value = 0,     max_value = 11000), # height in the atmosphere around which we will test
...
def test_eos_saturated(z, rnd, ql):
    ...
    def T_standard(z):
        return T0 + dTdz * z
    def p_standard(T):
        return 101300. / (1 + const.g * z / T / const.Rd)
